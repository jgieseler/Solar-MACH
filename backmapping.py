...
def make_the_plot(date, sc_list, vsw_list, flare_long):    
    ...
    color_dic = {'STEREO-A':'red', 'STEREO-B':'blue', 'Earth':'green', 'MPO': 'orange', 'PSP':'purple', 'SolarOrbiter':'dodgerblue'}
    for i, sc in enumerate(sc_list):
        ...
        alpha_f = np.deg2rad(delta_flare) + omega/(v_A/AU)*(dist_e/AU-r)-(omega/(v_A/AU)*(dist_e/AU))
    ...
    ax.plot(alpha_f, r, '--k')
    arr1 = plt.arrow(alpha_f[0], 0.01, 0, 1.1, head_width = 0.12, head_length = 0.11, edgecolor = 'black', facecolor = 'black', lw = 2, zorder = 5, overhang=0.2)
