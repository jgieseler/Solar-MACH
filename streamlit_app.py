...
def show_pfss_plots():
    ...
    if 'Reference flux tube lon range' in df_pfss2.columns:
        df_pfss['Reference flux tube lon range'] = df_pfss2['Reference flux tube lon range'].apply(lambda x: (float(round(x[0], decimals)), float(round(x[1], decimals)))).astype(str)
...
with st.sidebar.container():
    ...
    with st.sidebar.expander("Reference coordinates (e.g. flare)", expanded=st.session_state.plot_reference_check):
        ...
        def_reference_long = int(st.session_state["reference_long"][0]) if "reference_long" in st.session_state else 0
